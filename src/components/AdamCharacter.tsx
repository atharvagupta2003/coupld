...
export default function AdamCharacter({ size = 120, speaking = false }: Props) {
  const w = size
  ...
  return (
    <div className="relative inline-flex items-end justify-center">
      <motion.div
        ...
        transition={{ duration: speaking ? 0.45 : 4, repeat: Infinity, ease: 'easeInOut' }}
      ...
      >
        <div
          ...
          style={{
            borderRadius: 16,
            boxShadow: speaking
              ? '0 0 20px rgba(107,181,196,0.5), 0 8px 32px rgba(0,0,0,0.5)'
              : '0 8px 32px rgba(0,0,0,0.4)',
            border: speaking ? '2px solid rgba(107,181,196,0.6)' : '2px solid rgba(107,181,196,0.2)',
          }}
        >
          <img
            src={ADAM_PHOTO}
      ...
      {speaking && (
        <div className="absolute flex items-end gap-0.5" style={{ right: -12, top: '15%' }}>
          {[0, 1, 2, 3].map((i) => (
            <motion.div
              key={i}
              className="w-1 rounded-full bg-brand-lavender"
