...
function SocialButton({ icon, label, onClick }: { icon: ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center justify-center gap-2.5 w-full py-3 rounded-xl font-medium text-sm text-white transition-all active:scale-[0.98]"
      style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(184,160,212,0.18)' }}
    >
      {icon}
      {label}
    </button>
...
export default function SignUp() {
  ...
  return (
    ...
    >
      <SkipButton />
      <button onClick={() => navigate('/splash')} className="mb-8 text-white w-fit">
        <ChevronLeft size={24} />
      </button>
          ...
          {['Full name', 'Email address', 'Password'].map((label, i) => (
            <div key={label}>
              ...
              <input
                ...
                placeholder={label}
                className="w-full rounded-xl px-4 py-3 text-white text-sm outline-none placeholder:text-brand-textSub/50"
